...
if($isBtnSearchClicked == true) {
	...
	if($btnUpdateClicked == true) {
		...
		$sql = "UPDATE `dispatch` SET `time_arrived` = now() WHERE time_arrived is null and patrol_car_id = '" . $carId . "'";
		$updateSuccess = $conn->query($sql);
		...
		if($updateSuccess == false) {
			...
			$sql = "SELECT incident_id FROM `dispatch` WHERE time_completed is null and patrol_car_id = '" . $carId . "'";
			$result = $conn->query($sql);
			$incidentId = 0;
			...
			$sql = "UPDATE `dispatch` SET `time_completed` = now() WHERE time_completed is null and patrol_car_id = '" . $carId . "'";
		$updateSuccess = $conn->query($sql);
